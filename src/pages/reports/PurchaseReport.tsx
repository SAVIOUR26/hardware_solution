...
import { Calendar, Download, RefreshCw, TrendingDown } from 'lucide-react';
...
interface ReportFilters {
  ...
  group_by: 'day' | 'product' | 'supplier' | 'none';
}
...
function PurchaseReport() {
  ...
  const loadReport = async () => {
    try {
      setLoading(true);
      const result = await window.api.reports.getPurchaseReport(filters);
      if (result.success) {
      ...
      {reportData?.summary && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="bg-gradient-to-br from-red-500 to-red-600 text-white rounded-lg p-4">
            ...
            <p className="text-2xl font-bold">{formatCurrency(reportData.summary.total_purchases_ugx)}</p>
          ...
          <div className="bg-gradient-to-br from-indigo-500 to-indigo-600 text-white rounded-lg p-4">
            <p className="text-sm opacity-90 mb-1">USD Purchases</p>
            <p className="text-2xl font-bold">
              ${reportData.summary.total_purchases_usd.toLocaleString()}
            </p>
          </div>
        </div>
      )}
      ...
      {loading ? (
                ...
                <tr className="bg-muted/50 border-b border-border">
                  ...
                  {filters.group_by === 'product' && (
                    <>
                      ...
                      <th className="text-right py-3 px-4 font-semibold">Qty Purchased</th>
                      <th className="text-right py-3 px-4 font-semibold">Total</th>
                      <th className="text-right py-3 px-4 font-semibold">Avg Purchase</th>
                    </>
                  )}
                  {filters.group_by === 'supplier' && (
                    <>
              ...
              <tbody>
                {reportData.rows.map((row: any, index: number) => (
                  <tr key={index} className="border-b border-border hover:bg-muted/30">
                    {filters.group_by === 'day' && (
                      <>
                        ...
                        <td className="py-3 px-4 text-right font-semibold">
                          {formatCurrency(row.total_purchases_ugx)}
                        </td>
                        <td className="py-3 px-4 text-right">{row.invoice_count}</td>
                        <td className="py-3 px-4 text-right">
                          {formatCurrency(row.average_purchase_ugx)}
                        </td>
                      </>
                    )}
                    {filters.group_by === 'product' && (
                      <>
                        ...
                        <td className="py-3 px-4 text-right">{row.quantity_purchased}</td>
                        <td className="py-3 px-4 text-right font-semibold">
                          {formatCurrency(row.total_purchases_ugx)}
                        </td>
                        <td className="py-3 px-4 text-right">
                    ...
                    {filters.group_by === 'supplier' && (
                      <>
                        <td className="py-3 px-4 font-medium">{row.supplier_name}</td>
                        <td className="py-3 px-4 text-right font-semibold">
                          {formatCurrency(row.total_purchases_ugx)}
                        </td>
                        <td className="py-3 px-4 text-right">{row.invoice_count}</td>
        ...
        <div className="bg-card border border-border rounded-lg p-12 text-center">
          ...
          <p className="text-muted-foreground">
            No purchase data available for the selected period. Try adjusting your filters.
          </p>
        </div>
      )}
    </div>
